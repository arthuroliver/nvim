...
M.general = {
  ...
  n = {
    ...
    -- Copy all
    ["<C-c>"] = { "<cmd> :CopilotChatOpen <CR>", "Open Copilot Chat" },
...
M.nvimtree = {
  ...
  n = {
    ...
    -- Focus NvimTree or return to previous window
    ["<leader>e"] = {
      function()
        local view = require('nvim-tree.view')
        if view.is_visible() then
          if vim.api.nvim_get_current_win() == view.get_winnr() then
            vim.cmd('wincmd p')  -- Go to the previously focused window
          else
            view.focus()  -- Focus on NvimTree
          end
        else
          vim.cmd('NvimTreeToggle')  -- Open NvimTree if it is not open
        end
      end,
      "Toggle focus between NvimTree and Vim"
    },
  },
}
